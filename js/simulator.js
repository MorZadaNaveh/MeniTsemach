...
function runSimulator(){
  ...
  setTimeout(()=>{
    ...
    document.getElementById('simTeam').innerHTML=(t.teamReq||[]).map((r,i)=>{
      const tm=teamMembers.length>0?teamMembers[i%teamMembers.length]:null;
      return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;padding:7px 9px;background:var(--bg2);border:1px solid var(--s5);border-radius:7px">
        ${tm?`<div style="width:24px;height:24px;border-radius:5px;background:${tm.color};display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:700;color:#fff;flex-shrink:0">${tm.ini}</div>
        <div style="flex:1"><div style="font-size:12px;font-weight:700">${tm.name}</div><div style="font-size:10px;color:var(--s2)">${r.role}</div></div>`
        :`<div style="flex:1"><div style="font-size:12px;font-weight:700">${r.role}</div><div style="font-size:10px;color:var(--s3)">לא שובץ</div></div>`}
      </div>`;
    }).join('');
    document.getElementById('simRisks').innerHTML=`
      ...
      ${(t.highlights||[]).map(h=>`<div style="display:flex;gap:7px;margin-bottom:6px;font-size:11.5px">💡 ${h}</div>`).join('')||'<div style="font-size:11.5px;color:var(--s3)">אין המלצות ספציפיות</div>'}`;

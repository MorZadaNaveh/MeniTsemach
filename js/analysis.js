...
function downloadAppendixPdf(appIdx, tenderId){
  ...
  // Clone the DOM so we can bake input values into static text
  const clone = adoc.cloneNode(true);

  // Bake input values: replace <input> with styled <span>
  clone.querySelectorAll('input.fi, input[type="text"], input[type="number"], input[type="date"]').forEach(inp => {
    const original = adoc.querySelector(`[data-app="${inp.dataset.app}"][data-field="${inp.dataset.field}"]`);
    const val = original ? original.value : inp.getAttribute('value') || '';
    const span = document.createElement('span');
    span.style.cssText = 'display:block;padding:5px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px;min-height:20px;background:#fafafa;direction:rtl';
    span.textContent = val;
    inp.replaceWith(span);
  });

  // Bake textarea values
  clone.querySelectorAll('textarea').forEach(ta => {
    const original = adoc.querySelector(`textarea[data-app="${ta.dataset.app}"][data-field="${ta.dataset.field}"]`);
    const val = original ? original.value : ta.textContent || '';
    const div = document.createElement('div');
    div.style.cssText = 'padding:5px 8px;border:1px solid #ddd;border-radius:6px;font-size:12px;min-height:40px;background:#fafafa;white-space:pre-wrap;direction:rtl';
    div.textContent = val;
    ta.replaceWith(div);
  });

  // Bake contenteditable td values
  clone.querySelectorAll('td[contenteditable]').forEach(td => {
    const original = adoc.querySelector(`td[data-app="${td.dataset.app}"][data-row="${td.dataset.row}"][data-field="${td.dataset.field}"]`);
    if(original) td.textContent = original.textContent;
    td.removeAttribute('contenteditable');
  });

  // Remove buttons
  clone.querySelectorAll('.no-pdf').forEach(b => b.remove());

  // Temporarily add clone to body for html2pdf to measure
...
/* ── Form mode only — PDF generated from HTML via html2pdf ── */
